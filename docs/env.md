...
- `C2D_METRICS_INTERVAL_SECONDS`: How often (in seconds) the node samples live Docker runtime metrics (CPU, RAM, disk, network, block I/O, PIDs, exit info — plus NVIDIA GPU utilization/memory) for running compute jobs and services, persisting a snapshot onto the job record in the C2D database. These metrics are **node-internal by default**: they are never included in the escrow claim proof and are stripped from ordinary responses. They are exposed only on request, to the authenticated owner, via the `includeMetrics` flag on `COMPUTE_GET_STATUS` / `SERVICE_GET_STATUS` (see [API.md](API.md)); the node-wide `serviceList` never returns them. Set to `0` to disable collection entirely. Metrics are best-effort (up to one interval of staleness). Defaults to `10`. Example: `10`
