#!/bin/bash
# Muse-Glimmer-30B (vLLM) + Open WebUI in one container, one escrow.
#
...
# Qwen3.8 sibling at the same bucket, and two containers writing one Open WebUI sqlite file
# corrupts it, while two racing to create one venv leaves it half-built. The Hugging Face
# cache below IS shared — huggingface_hub locks correctly and the reuse is worth it.
VENV="$BUCKET/owui-venv-muse"
OWUI_DATA="$BUCKET/openwebui-muse"

echo "[ocean] $(id)"

# ---------------------------------------------------------------------------------------
# Gate 1: the bucket. It holds 34 GB of weights, the Open WebUI virtualenv and Open WebUI's
...
if [ ! -d "$BUCKET" ]; then
  echo "[ocean] no persistent-storage bucket mounted at $BUCKET — refusing to start." \
    "34 GB of weights would download into the container, and your chat history and admin" \
...
# tool use this model is strongest at.
#
