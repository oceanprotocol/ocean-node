#!/bin/bash
# Muse-Glimmer-30B (vLLM) + OpenCode in one container, one escrow.
#
...
# Qwen3.8 sibling at the same bucket, and two containers writing one OpenCode session store
# corrupts it. The Hugging Face cache below IS shared — huggingface_hub locks correctly and
# the reuse is worth it.
OC_HOME="$BUCKET/opencode-home-muse"        # becomes $HOME: binary, config, sessions, caches
WORKSPACE="$BUCKET/workspace-muse"          # the project directory OpenCode operates on

echo "[ocean] $(id)"

# ---------------------------------------------------------------------------------------
# Gate 1: the bucket. It holds 34 GB of weights, the OpenCode binary and every session,
...
if [ ! -d "$BUCKET" ]; then
  echo "[ocean] no persistent-storage bucket mounted at $BUCKET — refusing to start." \
    "34 GB of weights would download into the container, and the workspace OpenCode edits" \
...
# ---------------------------------------------------------------------------------------
# Reasoning strength. Unlike the Qwen3.8 sibling, this model has no reasoning_effort
# parameter — its card grades thinking through a line in the system prompt, and recommends
# high or xhigh for coding and agentic work. OpenCode appends the project's AGENTS.md to the
# system prompt, so that line goes there. Written only when absent, so editing it sticks.
# ---------------------------------------------------------------------------------------
if [ ! -e "$WORKSPACE/AGENTS.md" ]; then
  cat > "$WORKSPACE/AGENTS.md" <<'MD'
Reasoning strength: high

<!-- The line above is how Muse-Glimmer's thinking budget is set: it reads it from the
system prompt, and this file is appended to it. Valid values are low, medium, high and
xhigh; the model card recommends high or xhigh for coding and agentic work. Lower it to
trade quality for speed. Everything else you write in this file is ordinary project
instructions for the agent. -->
MD
  echo "[ocean] seeded $WORKSPACE/AGENTS.md with 'Reasoning strength: high'"
fi
...
#
# temperature and top_p are the model card's recommended sampling values and are forwarded
# in the request body. top_k 64 is not — the OpenAI-compatible client drops parameters it
# does not know — so it is set on `vllm serve` below instead.
# ---------------------------------------------------------------------------------------
...
config = {
    ...
    'provider': {
        'vllm': {
            ...
            'name': 'Muse-Glimmer-30B on this GPU (vLLM)',
            'options': {
            ...
            'models': {
                served: {
                    'name': 'Muse-Glimmer-30B FP8 (Apache-2.0, 131K context, vision)',
                    'attachment': True,
                    'reasoning': True,
                    'tool_call': True,
                    'temperature': True,
...
# tool use this model is strongest at, and leave OpenCode unable to call a single tool.
#
# --override-generation-config carries the card's top_k, which the OpenAI-compatible client
# cannot send, so it applies server-side to every request instead.
#
# --kv-cache-dtype is deliberately left at the model dtype: unlike the Qwen3.8 sibling there
# is no published fp8-KV guidance for this architecture, and the cache is cheap anyway (13 of
# 52 layers use full attention; the other 39 are capped by a 2048-token sliding window), so
# there is nothing to buy by quantising it.
# ---------------------------------------------------------------------------------------
vllm serve "$MODEL" \
  ...
  --max-num-seqs 16 \
  --override-generation-config '{"top_k":64}' \
