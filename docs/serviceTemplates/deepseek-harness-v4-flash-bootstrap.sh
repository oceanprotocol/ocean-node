...
#
# The proxy speaks HTTP, not HTTPS, because the node publishes a plain TCP port. The basic-auth
# password therefore crosses the network in cleartext (base64, which is not encryption) and so
# does everything the agent shows you. Treat the password as burnable, use a fresh one per
# launch, and put a TLS terminator in front if the node does not provide one. The container
# boundary — not the password — is the real isolation here.
#
...
#   NOT verified, and flagged again where it matters: that H200-class Hopper cards serve this
#   checkpoint under plain tensor parallelism. vLLM's recipe lists H200 only under a
#   prefill/decode-disaggregated deployment, and the fp4 expert weights are a Blackwell-native
#   format that Hopper must handle some other way. The warm-up gate below exists precisely so
#   this is discovered in seconds by the script rather than in hours by the consumer.
set -euo pipefail
...
PROXY_PORT=8080       # the published port (Caddy + basic auth)
...
# The parallel layout. 166.9 GB of weights does not fit on one 141 GB card, so TP 2 is the
# floor: ~83.5 GB of weights per card, leaving ~43 GB each for KV cache and activations at the
# utilisation below. Tensor parallelism splits by whole attention heads and the model has 64,
# so 2/4/8 are legal and 6 is not — do not "use all six cards" by setting 6, vLLM will refuse
# to start. Raise to 4 for more KV cache (a 1M-token context) at the cost of two more cards.
...
- `0.0.0.0:8080` — **the only published port.** A Caddy reverse proxy with HTTP basic auth in
  front of dsh.
...
if you are asked to "expose" something, expose it through the existing proxy or say no. The
connection is plain HTTP, so treat anything typed into this session as sent in cleartext.
...
# ---------------------------------------------------------------------------------------
# The published port: Caddy, basic auth, reverse proxy to dsh.
#
...
#
# `auto_https off` because the node publishes a plain TCP port and Caddy must not try to
# provision a certificate for a hostname it does not own. That is also why the password
# crosses the network in cleartext — see the security note at the top of this file.
#
# The password reaches Caddy as a bcrypt hash through the config file, and the plaintext is
...
echo "[ocean] ready — open the service URL and sign in as '$DSH_WEB_USERNAME' with the" \
  "password you set. In dsh: click Choose workspace and pick $WORKSPACE (the session composer" \
  "needs a workspace selected), then start a session — the model is already configured and" \
