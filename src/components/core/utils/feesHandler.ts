...
export class ProviderFees {
  ...
  getEnvironmentPriceSchemaForResource(
  ...
  calculateProviderFeeAmount(
    ...
    const providerFeeAmount = this.node.getConfig().feeStrategy?.feeAmount?.amount || 0
  ...
  async createProviderFee(
    ...
    const providerWallet = await this.getProviderWallet(String(assetChainId))
    ...
    const providerFeeToken = this.getProviderFeeToken(assetChainId)
    if (providerFeeToken?.toLowerCase() === ZeroAddress) {
      providerFeeAmount = 0
    } else {
      providerFeeAmount = this.calculateProviderFeeAmount(
        validUntil,
  ...
  async verifyProviderFees(
    ...
    const providerWallet = await this.getProviderWallet(String(chainId))
  ...
  async createFee(
    ...
    //   "id": service.id,
    // }
    const providerWallet = await this.getProviderWallet(String(asset.chainId))
    const providerFeeAddress: string = providerWallet.address

    // from env FEE_TOKENS
    const providerFeeToken: string = this.getProviderFeeToken(asset.chainId)

    // from env FEE_AMOUNT
    const providerFeeAmount: number = this.getProviderFeeAmount() // TODO check decimals on contract?
 ...
 * 
  ...
  async checkFee(
    ...
    const wallet = await this.getProviderWallet()
    const nodeAddress = wallet.address

    // first check if these are a match
    if (
      nodeAddress?.toLowerCase() !== providerFeesData.providerFeeAddress?.toLowerCase()
    ) {
      return false
    }
  ...
  async getProviderWalletAddress(): Promise<string> {
   ...
   * @returns amount
   */
  getProviderFeeAmount(): number {
    return this.node.getConfig().feeStrategy?.feeAmount?.amount || 0
  }
