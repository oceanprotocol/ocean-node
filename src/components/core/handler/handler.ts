...
export abstract class Handler implements ICommandHandler {
  ...
  async checkRateLimit(): Promise<boolean> {
    try {
      ...
      CORE_LOGGER.error(`Error checking rate limit: ${error.message}`)
      return false
    }
  }
  ...
  checkRequestData(
    ...
    try {
      ...
      CORE_LOGGER.error(`Error checking request data: ${error.message}`)
      return {
        valid: false,
  ...
  async verifyParamsAndRateLimits(task: Command): Promise<P2PCommandResponse> {
    try {
      ...
      CORE_LOGGER.error(`Error verifying parameters and rate limits: ${error.message}`)
      return {
        stream: null,
        status: { httpStatus: 500, error: 'Unknown error: ' + error }
      }
    }
  }
