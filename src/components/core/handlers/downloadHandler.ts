...
import { Handler, checkNonce, NonceResponse } from './aHandler.js'
...
export class DownloadHandler extends Handler {
  public constructor(task: any, config: OceanNodeConfig, db: Database) {
    ...
    if (!this.isDownloadCommand(task) && !this.isDownloadUrlCommand(task)) {
      throw new Error(
  ...
  // Put it in DownloadHandler to avoid ReferenceError: Cannot access 'Handler' before initialization
  async handleDownloadUrlCommand(): Promise<P2PCommandResponse> {
  ...
  async handle(): Promise<P2PCommandResponse> {
    ...
    try {
      ...
      return await this.handleDownloadUrlCommand()
    } catch (e) {
