 ...
 *
 * These live in their own module because **both ends of the protocol use them** — the
 * responder in `handleProtocolCommands.ts` and the client in `index.ts` — and because the
 * four constants only make sense as a set: each one is derived from the others, and moving
 * one without the rest reintroduces the silent-discard failure they exist to prevent.
 * Importing framing from the responder file, as the client used to, said the wrong thing
 * about which of the two owns it. Neither does.
 *
 * Kept byte-for-byte in step with ocean.js's equivalent module. Divergence between the two
 * is not a style question: it is a peer reading a different number of bytes than the other
 * one wrote.
 */
