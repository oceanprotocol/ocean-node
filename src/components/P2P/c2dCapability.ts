 ...
 *
 * This module is intentionally pure: no network access, no libp2p, no I/O, no dependency on
 * anything else in this package. It only turns a typed capability into the exact byte string
 * that gets hashed into a content id.
 *
 * The query side of a search lives in a separate package that this repository does not (and
 * must not) depend on, so that package computes the same content id from an independently
 * written copy of this same rule. Identical bytes on both sides matter more than almost
 * anything else in this file — if the two implementations ever disagree, a search silently
 * returns zero providers, with no error anywhere. Nothing here should ever be "obviously
 * equivalent" to the other side; it must be identical, which is why every value below is
 * pinned by a golden-vector test that is mirrored, row for row, in the other package.
 */
 ...
 * source of truth for it — nothing else in this codebase (or in the cooperating implementation
 ...
 * rather than left to however the JS engine would order the keys of an object literal: a
 * resource name that happens to look like an array index (e.g. a purely numeric string) would
 * otherwise be reordered ahead of `free` by the engine's own integer-key ordering rules,
 * silently changing the bytes that get hashed. Building the string directly, field by field,
 ...
 * module, on either side. Adding an entry here changes what bytes get hashed for that
 * resource, which is a breaking format change like any other, not a routine addition.
 *
 ...
 *
 * Both the announce side and the query side of a search call this same function (in their own
 * independent implementations), and both round down: an announcer emits every bucket at or
 * below its true maximum, and a search request is bucketed down to the nearest one it could
 * plausibly find. That produces false positives (a query for `9` buckets to `8` and matches a
 * provider whose real maximum is exactly `8`) and never false negatives (a provider is never
 * excluded because its own bucket rounded below what it can actually do). Missing a capable
 * provider is worse than finding one that turns out, on closer inspection, not to qualify —
 * which is exactly why a caller must verify a candidate against its real capabilities before
 * trusting a bucket match; the bucket is a coarse prefilter, not an answer by itself.
 */
export function c2dBucketFor(resource: string, value: number): number {
  ...
  // Own properties only: a resource named after an inherited `Object.prototype` member
  // (`constructor`, `toString`, ...) must not be treated as an override ladder - that entry is a
  // function, not an array, and iterating it below would throw.
  const ladder = Object.hasOwn(C2D_BUCKET_OVERRIDES, resource)
