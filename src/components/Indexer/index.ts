...
export class OceanIndexer {
  ...
  public getSupportedNetwork(chainId: number): SupportedNetwork {
    try {
      ...
      INDEXER_LOGGER.error(`Error getting supported network: ${error}`)
    }
  }
  ...
  public stopAllThreads(): boolean {
    try {
      ...
      INDEXER_LOGGER.error(`Error stopping all threads: ${error}`)
      return false
    }
  }
  ...
  public stopThread(chainID: number): boolean {
    try {
      ...
      INDEXER_LOGGER.error(`Error stopping thread for chain ${chainID}: ${error}`)
      return false
    }
  }
  ...
  public startThread(chainID: number): Worker | null {
    try {
      ...
      INDEXER_LOGGER.error(`Error starting thread for chain ${chainID}: ${error}`)
      return null
    }
  }
  ...
  public startThreads(): boolean {
    try {
      ...
      INDEXER_LOGGER.error(`Error starting threads: ${error}`)
      return false
    }
  }
  ...
  public addReindexTask(reindexTask: ReindexTask): JobStatus | null {
    try {
      ...
      INDEXER_LOGGER.error(`Error adding reindex task: ${error}`)
    }
  }
  ...
  public resetCrawling(chainId: number): JobStatus | null {
    try {
      ...
      INDEXER_LOGGER.error(`Error resetting crawling: ${error}`)
    }
  }
  ...
  public getIndexingQueue(): ReindexTask[] {
    try {
      ...
      INDEXER_LOGGER.error(`Error getting indexing queue: ${error}`)
    }
  }
  ...
  public getJobsPool(jobId?: string): JobStatus[] {
    try {
      ...
      INDEXER_LOGGER.error(`Error getting jobs pool: ${error}`)
    }
  }
  ...
  public addJob(jobInfo: JobStatus) {
    try {
      ...
      INDEXER_LOGGER.error(`Error adding job: ${error}`)
    }
  }
  ...
  private filterJobs(jobId: string): { position: number; jobsResult: JobStatus[] } {
    try {
      ...
      INDEXER_LOGGER.error(`Error filtering jobs: ${error}`)
    }
  }
