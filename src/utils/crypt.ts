...
export async function encrypt(
  ...
  if (algorithm === EncryptMethod.AES) {
    ...
    // cast to any to satisfy TypeScript overloads expecting BinaryLike / ArrayBufferView
    const update = cipher.update(data)
    const field = cipher.final()
...
export async function decrypt(
  ...
  if (algorithm === EncryptMethod.AES) {
    ...
    decryptedData = Buffer.concat([
      new Uint8Array(update.buffer, update.byteOffset, update.byteLength),
      new Uint8Array(final.buffer, final.byteOffset, final.byteLength)
    ])
  } else if (algorithm === EncryptMethod.ECIES) {
    const sk = new eciesjs.PrivateKey(privateKey)
    decryptedData = eciesjs.decrypt(
      new Uint8Array(sk.secret.buffer, sk.secret.byteOffset, sk.secret.byteLength),
      data
    )
  }
